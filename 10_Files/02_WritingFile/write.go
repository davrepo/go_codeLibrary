...
func writeFileData() {
	...
	// the Sync function forces the data to be written out to disc
	f.Sync()
}

func appendFileData(fname string, data string) {
	// there is no append function in GO
...
func main() {
	...
	if !checkFileExists("testfile.txt") {
		...
		// data is now "This is a ", because we truncated the file to 10 bytes
