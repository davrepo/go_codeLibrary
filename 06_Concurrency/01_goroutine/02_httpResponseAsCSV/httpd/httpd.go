...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	// Fprintf writes to http.ResponseWriter (w) the formatted string
	// the header of the csv file
	fmt.Fprintf(w, "start,end,distance,passengers\n")

	start := day
	nextDay := day.Add(24 * time.Hour)
	// start.Before(nextDay) is a condition that's true when start is before nextDay
	for start.Before(nextDay) {
		start = start.Add(3 * time.Second) // start incremented by 3 seconds
		// thus for each 3 seconds we write a line in the csv file
