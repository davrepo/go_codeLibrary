...
import (
	...
	"example.com/backend" // the package we're testing
)

// instantiate the backend with a test database
var a backend.App
...
func TestMain(m *testing.M) {
	// instantiate the application variable
	a = backend.App{}
	a.Initialize()
	ensureTableExists()
	code := m.Run()

	clearProductTable() // for other tests we might do
	clearOrderTable()
	clearOrderItemTable()
	os.Exit(code)
}
...
func ensureTableExists() {
	// want to check for an error, dont want anything returns
...
func clearProductTable() {
	...
	a.DB.Exec("DELETE FROM sqlite_sequence WHERE name = 'products'") // reset the auto increment
}

func clearOrderTable() {
	a.DB.Exec("DELETE FROM orders")
	a.DB.Exec("DELETE FROM sqlite_sequence WHERE name = 'orders'") // reset the auto increment
}
...
func TestCreateOrderItem(t *testing.T) {
	...
	// order_id and product_id are composite primary keys in order_items table
	// are also foreign keys to orders and products tables
...
func executeRequest(req *http.Request) *httptest.ResponseRecorder {
	// httptest is a package that lets us test HTTP requests
