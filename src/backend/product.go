...
func (p *product) getProduct(db *sql.DB) error {
	// with the correct p.ID, we scan for the rest of the fields
...
func (p *product) createProduct(db *sql.DB) error {
	resp, err := db.Exec("INSERT INTO products(productCode, name, inventory, price, status) VALUES(?, ?, ?, ?, ?)", p.ProductCode, p.Name, p.Inventory, p.Price, p.Status)
	if err != nil {
		return err
	}
