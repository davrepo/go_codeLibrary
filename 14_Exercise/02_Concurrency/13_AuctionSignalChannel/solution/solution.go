...
func (a *auctioneer) openBids(open chan<- struct{}) {
	for i := 0; i < bidderCount; i++ {
		open <- struct{}{} // signal to open bids
	}
}
...
func (a *auctioneer) processWinner(item string, bids <-chan bid) {
	var winner bid
	for i := 0; i < bidderCount; i++ {
		cb := <-bids // read all bids from the bids channel
...
func (b *bidder) placeBid(out chan<- bid, open <-chan struct{}) {
	for i := 0; i < len(items); i++ {
		<-open // wait for the auctioneer openBid() to open the bids
		currentBid := bid{bidderID: b.id}
		if b.wallet > 0 {
			currentBid.amount = getRandomAmount(b.wallet)
		}
		out <- currentBid // place bid on the bids channel
	}
}
...
func main() {
	...
	// bids is buffered channel that all bidders will use to place their bids
	bids := make(chan bid, bidderCount)
	openBids := make(chan struct{}) // signal channel to open bids
	...
	for i := 0; i < bidderCount; i++ {
		...
		bidders[id] = &b
		go b.placeBid(bids, openBids) // channels passed here
	}
	a := auctioneer{
		bidders: bidders,
	}
