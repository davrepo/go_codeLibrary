...
// spread the gossip to all friends
// Graph traversal
...
func main() {
	// friends is a struct, where fmap is a field of type map, key string(Friend.ID), value Friend struct
	friends := importData()
	// root is a Friend struct
...
func importData() Friends {
	...
	// creates a map match the id to the friend struct
	fm := make(map[string]Friend, len(data))
	for _, d := range data {
		fm[d.ID] = d
	}

	// fmap is a field of Friends struct, see above
	// NB! Friends is not Friend
	return Friends{
		fmap: fm,
	}
}
